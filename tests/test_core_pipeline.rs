...
fn test_opus_codec_roundtrip() {
    ...
    encoder.set_bitrate(24000).expect("bitrate set failed");
    ...
    let len = encoder.encode(&input, &mut encoded).expect("encoding failed");
    ...
    let decoded_len = decoder.decode(&encoded, &mut decoded, false)
    ...
    assert!(avg_diff < 500); // Reasonable threshold for voice
}
...
fn test_rtp_serialization_logic() {
    ...
    assert_eq!(u32::from_be_bytes([packet[4], packet[5], packet[6], packet[7]]), timestamp);
...
fn test_opus_packet_loss_concealment() {
    ...
    let decoded = decoder.decode(&[], &mut output, true)
        .expect("PLC failed");
